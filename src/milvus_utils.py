...
def connect() -> None:
    ...
    if SETTINGS.milvus_token:
        kwargs["token"] = SETTINGS.milvus_token               # Serverless (token)
        # não passar db_name, não criar DB
    else:
...
def _schema(dim: int) -> CollectionSchema:
    return CollectionSchema(
        fields=[
            FieldSchema(name="id", dtype=DataType.INT64, is_primary=True, auto_id=True),  # <<< PK
